...
def mock_db_session():
    ...
    with patch("src.api.main.db.session_factory", return_value=MockDBContext()) as mock_session:
        yield mock_session
...
def mock_tenant():
    ...
    with patch("src.api.main.get_default_tenant", new_callable=AsyncMock, return_value=MockTenant()) as mock:
        yield mock
...
def mock_log_attack():
    ...
    with patch("src.api.main.log_attack", new_callable=AsyncMock) as mock:
        yield mock
...
def mock_mlx_infer():
    ...
    with patch("src.ml_engine.local_inference.mlx_model.infer", new_callable=AsyncMock) as mock:
        yield mock
...
def mock_bilstm_predict():
    ...
    with patch("src.api.pipeline.bilstm_model.predict") as mock:
        yield mock
