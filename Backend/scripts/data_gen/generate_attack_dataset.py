#!/usr/bin/env python3
"""
Synthetic Attack Dataset Generator for Chameleon LSTM Model
============================================================

Uses DeepSeek API to generate realistic shell command sequences.
Generates 1,000 rows with balanced classes (500 benign, 500 malicious).

CSV columns: timestamp, attacker_ip, label, command_sequence
- label 0 = Benign
- label 1 = Malicious
"""
...
OUTPUT_FILE = 'data/raw/custom_attack_data.csv'
...
def main():
    print("=" * 70)
    print(" CHAMELEON SYNTHETIC DATASET GENERATOR")
    print(" Target: 1,000 rows (500 benign + 500 malicious)")
    print("=" * 70)
    ...
    print("\n[Phase 1] Generating Benign Commands...")
    print("-" * 50)
    
    for i in range(10):
        print(f"  API call {i+1}/10 for benign commands...")
        commands = generate_benign_commands()
        if commands:
            benign_commands.extend(commands)
            print(f"    → Got {len(commands)} commands (total: {len(benign_commands)})")
        else:
            print("    → Using static fallback")
            benign_commands.extend(random.sample(generate_static_benign(), min(5, len(generate_static_benign()))))
        time.sleep(0.5)
    
    print(f"\n  Total benign commands collected: {len(benign_commands)}")
    
    print("\n[Phase 2] Generating Malicious Commands...")
    print("-" * 50)
    
    for i in range(10):
        print(f"  API call {i+1}/10 for malicious commands...")
        commands = generate_malicious_commands()
        if commands:
            malicious_commands.extend(commands)
            print(f"    → Got {len(commands)} commands (total: {len(malicious_commands)})")
        else:
            print("    → Using static fallback")
            malicious_commands.extend(random.sample(generate_static_malicious(), min(5, len(generate_static_malicious()))))
        time.sleep(0.5)
    
    print(f"\n  Total malicious commands collected: {len(malicious_commands)}")
    
    static_benign = generate_static_benign()
    static_malicious = generate_static_malicious()
    
    while len(benign_commands) < 500:
        benign_commands.extend(random.sample(static_benign, min(10, len(static_benign))))
    benign_commands = benign_commands[:500]
    
    while len(malicious_commands) < 500:
        malicious_commands.extend(random.sample(static_malicious, min(10, len(static_malicious))))
    malicious_commands = malicious_commands[:500]
    
    print("\n[Phase 3] Creating Dataset...")
    print("-" * 50)
    ...
    for i in range(500):
        dataset.append({
            'timestamp': generate_timestamp(base_time, (500 + i) * 60),
            'attacker_ip': generate_random_ip(),
    ...
    print(f"  Benign (0):     {sum(1 for r in dataset if r['label'] == 0)}")
    print(f"  Malicious (1):  {sum(1 for r in dataset if r['label'] == 1)}")
    print(f"  Output file:    {OUTPUT_FILE}")
    
    print("\n[Sample Data]")
    print("-" * 70)
    print("Benign examples:")
    for i, row in enumerate([r for r in dataset if r['label'] == 0][:3]):
        print(f"  {i+1}. {row['command_sequence'][:70]}...")
    
    print("\nMalicious examples:")
    for i, row in enumerate([r for r in dataset if r['label'] == 1][:3]):
        print(f"  {i+1}. {row['command_sequence'][:70]}...")
